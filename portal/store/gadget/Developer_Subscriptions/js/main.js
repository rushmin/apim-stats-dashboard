...
function getColumns() {
  columns = [{"name":"Subscription Count","type":"INTEGER"},{"name":"Time","type":"STRING"}];
};
...
function fetchData(callback) {
    ...
    $.ajax({
        url: "/portal/apis/api-stats/subscriptionsOverTime",
        method: "GET",
...
function makeRows(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var record = data[i];
        var row = [record.subscriptionCount, record.time];
        rows.push(row);
    };
    return rows;
};
