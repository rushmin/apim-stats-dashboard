...
function getColumns() {
  columns = [{"name":"Subscriptions Count","type":"INTEGER"},{"name":"API Name","type":"STRING"}];
};
...
function fetchData(callback) {
    ...
    $.ajax({
        url: "/portal/apis/api-stats/hot-apis",
        method: "GET",
...
function makeRows(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var record = data[i];
        var row = [record.subscriptionCount, record.apiName];
        rows.push(row);
    };
    return rows;
};
